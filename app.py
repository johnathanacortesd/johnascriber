    ...
    try:
        if file_ext in ['.mp4', '.mpeg', '.webm', '.avi', '.mov', '.mkv']:
            with VideoFileClip(input_path) as video:
                ### MEJORA: Se eliminan parámetros conflictivos como nbytes para evitar el error 'video_fps'.
                video.audio.write_audiofile(output_path, codec='libmp3lame', bitrate=target_bitrate, fps=16000, logger=None)
        else:
            with AudioFileClip(input_path) as audio:
                ### MEJORA: Se eliminan parámetros conflictivos como nbytes para evitar el error 'video_fps'.
                audio.write_audiofile(output_path, codec='libmp3lame', bitrate=target_bitrate, fps=16000, logger=None)
...
# --- FUNCIÓN ROBUSTA PARA LLAMADAS A LA IA (Sin cambios) ---
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.status_code in [429, 503] and attempt < max_retries - 1:
                wait_time = 2 ** attempt
                st.warning(f"🤖 Modelo '{model}' sobrecargado. Reintentando en {wait_time}s... (Intento {attempt + 1}/{max_retries})")
...
# --- FUNCIONES DE ANÁLISIS (ADAPTADAS PARA ACEPTAR EL MODELO SELECCIONADO) ---
def post_process_with_llama(transcription_text, client, model_name):
    messages = [
...
**REGLAS INVIOLABLES:**
1.  **ACENTUACIÓN PRECISA:** Añade tildes faltantes (ej: `como` -> `cómo`, `mas` -> `más`).
2.  **COMPLETAR PALABRAS:** Únicamente completa terminaciones obvias (ej: `informaci` -> `información`).
3.  **NO CAMBIAR PALABRAS VÁLIDAS:** No modifiques palabras que ya son correctas.
4.  **PROHIBIDO INVENTAR, OMITIR O REESCRIBIR:** No añadas, elimines ni reordenes palabras.
5.  **DEVOLVER TEXTO ÍNTEGRO:** Devuelve siempre el texto completo con solo las correcciones permitidas.
Tu salida debe ser únicamente el texto corregido."""},
        {"role": "user", "content": f"Aplica tus reglas de corrección a la siguiente transcripción:\n\n{transcription_text}"}
    ]
    corrected_text = robust_llama_completion(client, messages, model=model_name, temperature=0.0, max_tokens=4096)
    return corrected_text if corrected_text else transcription_text

def generate_summary(transcription_text, client, model_name):
    messages = [
    ...
    summary = robust_llama_completion(client, messages, model=model_name, temperature=0.3, max_tokens=500)
    return summary if summary else "No se pudo generar el resumen debido a un error."

def answer_question(question, transcription_text, client, conversation_history, model_name):
    messages = [{"role": "system", "content": "Eres un asistente experto. Responde preguntas sobre la transcripción de manera precisa y concisa, basándote ÚNICAMENTE en la información proporcionada. Si la información no está en el texto, indícalo claramente."}]
    for qa in conversation_history:
        messages.append({"role": "user", "content": qa["question"]})
        messages.append({"role": "assistant", "content": qa["answer"]})
    messages.append({"role": "user", "content": f"Transcripción:\n---\n{transcription_text}\n---\nPregunta: {question}"})
    
    answer = robust_llama_completion(client, messages, model=model_name, temperature=0.2, max_tokens=800)
    return answer if answer else "No se pudo procesar la pregunta debido a un error."

def extract_people_and_roles(transcription_text, client, model_name):
    messages=[
...
2. Incluye el cargo/rol si se menciona; si no, usa "Rol no especificado".
3. Proporciona el contexto (frase exacta donde se menciona).
FORMATO DE SALIDA (JSON válido): { "personas": [ { "name": "Nombre", "role": "Cargo", "context": "Contexto" } ] }
Si no hay personas, devuelve: {"personas": []}'''},
        {"role": "user", "content": f"Analiza esta transcripción:\n\n{transcription_text[:3500]}"}
    ]
    response_json = robust_llama_completion(client, messages, model=model_name, temperature=0.0, max_tokens=1500, response_format={"type": "json_object"})
    try:
        return json.loads(response_json).get('personas', []) if response_json else []
    except (json.JSONDecodeError, TypeError): return []

def extract_brands_and_entities(transcription_text, client, model_name):
    messages=[
...
3. Proporciona el contexto EXACTO (la frase completa donde se menciona).
FORMATO DE SALIDA (JSON válido): { "entidades": [ { "name": "Nombre", "type": "Tipo", "context": "Contexto" } ] }
Si no hay entidades, devuelve: {"entidades": []}'''},
        {"role": "user", "content": f"Analiza esta transcripción:\n\n{transcription_text[:4000]}"}
    ]
    response_json = robust_llama_completion(client, messages, model=model_name, temperature=0.0, max_tokens=2000, response_format={"type": "json_object"})
    try:
...
with st.sidebar:
    ...
    model_option = st.selectbox("Modelo de Transcripción", ["whisper-large-v3"], help="El modelo más preciso disponible para transcripción.")
    ...
    ### MEJORA: Selector para el modelo de Llama
    llama_model_option = st.selectbox(
        "Modelo de Llama para Análisis",
        ["llama-3.1-8b-instant", "llama-3.3-70b-versatile"],
        help="Elige 'instant' para velocidad o 'versatile' para máxima calidad en el análisis."
    )
    
    enable_llama_postprocess = st.checkbox("Corrección IA de la transcripción", value=True, help="Usa Llama para corregir tildes y errores comunes.")
    ...
    if MOVIEPY_AVAILABLE:
        st.success("✅ **Optimización de Audio Activada:** Los archivos se convierten a MP3 optimizado para máxima velocidad y precisión.")
    else:
        ...
        try:
            ...
            with st.spinner("🔄 Transcribiendo con IA (modo de máxima precisión)..."):
                with open(tmp_path, "rb") as audio_file:
                    ### MEJORA: Se elimina el `prompt` para evitar que se transcriba.
                    # El modelo whisper-large-v3 es literal por defecto.
            ...
            if enable_llama_postprocess:
                with st.spinner("🤖 Mejorando transcripción con IA..."):
                    transcription_text = post_process_with_llama(transcription_text, client, llama_model_option)
            ...
            with st.spinner("🧠 Generando análisis avanzado..."):
                if enable_summary: st.session_state.summary = generate_summary(transcription_text, client, llama_model_option)
                if enable_people: st.session_state.people = extract_people_and_roles(transcription_text, client, llama_model_option)
                if enable_brands: st.session_state.brands = extract_brands_and_entities(transcription_text, client, llama_model_option)
...
# --- SECCIÓN DE RESULTADOS (Sin cambios mayores) ---
if 'transcription' in st.session_state:
    ...
    if 'people' in st.session_state and st.session_state.get('people'): tab_titles.append("👥 Personas Clave")
    if 'brands' in st.session_state and st.session_state.get('brands'): tab_titles.append("🏢 Marcas")
        ...
        if 'summary' in st.session_state:
            ...
            if st.session_state.qa_history:
                if st.button("🗑️ Borrar Historial", use_container_width=True):
                    st.session_state.qa_history = []
                    st.rerun()
        else: st.info("📝 El resumen no fue generado. Activa la opción en el sidebar y vuelve a transcribir.")
    ...
    if 'brands' in st.session_state and st.session_state.get('brands'):
        with tabs[tab_idx]:
            ...
            with col_brand_search1: brand_search_query = st.text_input("🔎 Buscar marca específica:", key="brands_search")
            ...
            if brand_search_query:
                ...
                st.info(f"{len(brands_to_show)} coincidencia(s) encontrada(s)." if brands_to_show else "No se encontraron marcas con ese nombre.")
            
            for brand in brands_to_show:
                brand_name = brand.get('name', 'N/A')
                st.markdown(f"**🏢 {brand_name}** | **Tipo:** *{brand.get('type', 'N/A')}*")
                    ...
                    if matches:
                        ...
                        for match_idx in matches:
                            ...
                            for ctx_seg in get_extended_context(segments, match_idx, context_lines):
                                ...
                                with col_time: st.button(f"▶️ {ctx_seg['time']}", key=f"brand_play_{brand_name}_{match_idx}_{ctx_seg['start']}", on_click=set_audio_time, args=(ctx_seg['start'],), use_container_width=True)
                                with col_text:
                                    style = MATCH_LINE_STYLE if ctx_seg['is_match'] else CONTEXT_LINE_STYLE
                                    text = re.compile(re.escape(brand_name), re.IGNORECASE).sub(f'<span style="{HIGHLIGHT_STYLE}">\g<0></span>', ctx_seg['text']) if ctx_seg['is_match'] else ctx_seg['text']
                                    st.markdown(f"<div style='{style}'>{text}</div>", unsafe_allow_html=True)
...
<div style='text-align: center; color: #666; margin-top: 2rem;'>
    <p><strong>Transcriptor Pro - Johnascriptor - v4.1.0</strong></p>
    <p style='font-size: 0.9rem;'>🎙️ whisper-large-v3 | 🤖 Llama 3.1 & 3.3 | 🎵 Conversión de Audio Robusta</p>
