    ...
    try:
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "Eres un corrector ortográfico experto en español. Corrige tildes, acentos y palabras cortadas en transcripciones de noticias, manteniendo el significado original."
                },
                {
                    "role": "user",
                    "content": f"Corrige la ortografía en español (agrega tildes donde falten, une palabras cortadas): {transcription_text}"
                }
            ],
            model="llama-3.3-70b-versatile",
            temperature=0.0,
    ...
    if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True, disabled=not uploaded_file):
        ...
        with st.spinner("🔄 Procesando archivo..."):
            try:
                ...
                with st.spinner("🔄 Transcribiendo con IA avanzada..."):
                    with open(tmp_file_path, "rb") as audio_file:
                        transcription = client.audio.transcriptions.create(
                            ...
                            prompt="Transcripción precisa de noticias en español. Incluye tildes y acentos correctamente: política, economía, México, España, América Latina."  # Mejora: Prompt para mejor manejo de español
                        )
                ...
                # Mejora: Corrección opcional con LLM
