...
if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True, disabled=not uploaded_file):
    # Limpiar estado de sesión anterior, manteniendo la autenticación
    for key in list(st.session_state.keys()):
        if key not in ['password_correct', 'password_attempted']: st.session_state.pop(key)
    st.session_state.audio_start_time = 0; st.session_state.transcription_id = st.session_state.get('transcription_id', 0) + 1; st.session_state.last_search = ""
...
if 'transcription' in st.session_state:
    ...
    with tabs[0]:
        ...
        st.markdown(f'<div style="{TRANSCRIPTION_BOX_STYLE}" key="transcription_box_{st.session_state.transcription_id}">{transcription_html}</div>', unsafe_allow_html=True)
        st.write("")
        col_d1, col_d2, col_d3, col_d4 = st.columns([2, 2, 2, 1.5])
        with col_d1: st.download_button("💾 Descargar TXT Simple", st.session_state.transcription.encode('utf-8'), "transcripcion.txt", use_container_width=True)
    ...
    with tabs[1]:
        if 'summary' in st.session_state: st.markdown("### 📝 Resumen Ejecutivo"); st.markdown(st.session_state.summary)
    ...
    if len(tabs) > 2:
        with tabs[2]:
                    ...
                    if data['mentions']:
                        with st.expander(f"Ver {len(data['mentions'])} mención(es) en el audio"):
                            for mention in data['mentions']:
                                ...
                                with col1: st.button(f"▶️ {mention['time']}", key=f"play_{name}_{mention['start']}", on_click=set_audio_time, args=(mention['start'],))
            ...
            if not people_data and not brands_data: st.info("No se identificaron personas o marcas. Activa la opción en el sidebar o puede que no existan en el audio.")
...
st.markdown("---")
st.markdown("""<div style='text-align: center; color: #666;'><p><strong>Transcriptor Pro - Johnascriptor - v3.4.0 (Whisper-large-v3 | Llama-3.1)</strong></p><p style='font-size: 0.85rem;'>✨ Optimizado para noticias en español de Colombia</p></div>""", unsafe_allow_html=True)
