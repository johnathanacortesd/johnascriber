...
def extract_brands(transcription_text, client):
    try:
        ...
        response_content = chat_completion.choices[0].message.content
        data = json.loads(response_content)
        # El modelo puede anidar la lista dentro de una clave, la buscamos
        if isinstance(data, list): return data
        for key in data:
            if isinstance(data[key], list): return data[key]
        return []
    ...
    if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True, disabled=not uploaded_file):
        ...
        with st.spinner("🔄 Procesando archivo..."):
            try:
                ...
                # La corrección por diccionario siempre se aplica a segmentos individuales para mantener la consistencia
                ...
                scroll_to_top() # MEJORA: Llama a la función para hacer scroll al inicio
                st.balloons()
            except Exception as e:
...
if 'transcription' in st.session_state and 'uploaded_audio_bytes' in st.session_state:
    ...
    if 'brands' in st.session_state: tab_titles.append("🏢 Marcas Mencionadas") # MEJORA: Añadir pestaña de marcas
    ...
    if 'people' in st.session_state:
        with tabs[tab_index]:
            ...
            if people_data and not ("Error" in people_data[0].get('name', '')):
                ...
                # MEJORA: Usar enumerate para crear una clave única para cada expander
    ...
    if 'brands' in st.session_state:
        with tabs[tab_index]:
            ...
            if brands_data and not ("Error" in brands_data[0].get('brand', '')):
                ...
                st.error(f"**{brands_data[0]['brand']}**: {brands_data[0]['context']}")
            else:
...
if st.button("🗑️ Limpiar Todo y Empezar de Nuevo"):
    keys_to_delete = [
        ...
        "search_counter", "people", "qa_history", "brands" # MEJORA: Limpiar estado de marcas
    ]
