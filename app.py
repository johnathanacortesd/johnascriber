...
def clean_whisper_artifacts(text):
    """Elimina artefactos y repeticiones que genera Whisper al final"""
    if not text:
        return text
    
    # Patrones problemáticos comunes
    patterns_to_remove = [
        r'(Palabras clave[.\s]*){2,}',  # "Palabras clave" repetido
        r'(Subtítulos[.\s]*){2,}',       # "Subtítulos" repetido
        r'(Música[.\s]*){3,}',           # "Música" repetido
        r'(Gracias[.\s]*){3,}',          # "Gracias" repetido
        r'(\[Música\][.\s]*){2,}',       # [Música] repetido
        r'(\.\s*){5,}',                   # Puntos excesivos
    ]
    ...
    # Limpiar puntos finales múltiples (AQUÍ ESTABA EL ERROR)
...
def fix_spanish_encoding_light(text):
    ...
    # Solo corrige errores de encoding UTF-8 malformado (Mojibake)
    replacements = {
...
def text_chunker_smart(text, chunk_size=3000):
    ...
    # Dividir por oraciones de forma más precisa, INCLUYENDO CARACTERES EN ESPAÑOL
...
if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True, disabled=not uploaded_file):
    ...
    try:
        ...
        with st.spinner("🔄 Transcribiendo con Whisper V3 (modo exacto)..."):
            ...
            # PREPARAR PROMPT - Sin palabras sueltas que puedan repetirse
            if use_custom_prompt and 'custom_prompt' in locals():
                whisper_prompt = custom_prompt
            else:
                # Prompt sin palabras sueltas que Whisper pueda repetir
                whisper_prompt = "Esta es una transcripción en español de alta calidad con puntuación correcta."
        ...
        # 3.5 LIMPIAR ARTEFACTOS DE WHISPER
