            ...
            try:
                ...
                with st.spinner("🔄 Transcribiendo con IA avanzada..."):
                    ...
                    with open(tmp_file_path, "rb") as audio_file:
                        # --- CÓDIGO CORREGIDO ---
                        # Se ha eliminado el parámetro 'word_timestamps' que no es soportado por Groq
...
if 'transcription' in st.session_state and 'uploaded_audio_bytes' in st.session_state:
    ...
    with tab1:
        ...
        TRANSCRIPTION_BOX_STYLE = "background-color: #0E1117; color: #FAFAFA; border: 1px solid #333; border-radius: 10px; padding: 1.5rem; max-height: 500px; overflow-y: auto; font-family: 'Source Code Pro', 'Consolas', monospace; line-height: 1.7; white-space: pre-wrap; font-size: 0.95rem;"

        col_search1, col_search2 = st.columns([4, 1])
        with col_search1:
            search_query = st.text_input("🔎 Buscar en la transcripción:", value=st.session_state.get('last_search', ''), placeholder="Escribe para encontrar y escuchar un momento exacto...", key=f"search_input_{st.session_state.get('search_counter', 0)}")
        ...
        if search_query:
            with st.expander("Resultados de la búsqueda contextual", expanded=True):
                ...
                if not matching_indices:
                    ...
                    for idx in matching_indices: indices_to_display.update(range(max(0, idx - 1), min(len(segments), idx + 2)))
                    last_index = -2
        ...
        with col_d1: st.download_button("💾 Descargar TXT Simple", st.session_state.transcription, "transcripcion.txt", "text/plain", use_container_width=True)
        with col_d2:
    ...
    if st.button("🗑️ Limpiar Todo y Empezar de Nuevo", type="secondary", use_container_width=False):
        keys_to_delete = ["transcription", "transcription_data", "uploaded_audio_bytes", "audio_start_time", "summary", "quotes", "last_search", "search_counter", "original_filename"]
        for key in keys_to_delete:
            if key in st.session_state: del st.session_state[key]
        st.rerun()

st.markdown("---")
st.markdown("""<div style='text-align: center; color: #666;'><p><strong>Transcriptor Pro - Johnascriptor - v2.0</strong> - Desarrollado por Johnathan Cortés 🤖</p></div>""", unsafe_allow_html=True)
