...
# --- NUEVA FUNCIÓN PARA EXTRAER MARCAS ---
def extract_brands(transcription_text, client):
    try:
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "Eres un analista experto en identificar marcas comerciales en textos. Tu tarea es extraer una lista de todas las marcas mencionadas. Devuelve el resultado como un objeto JSON con una única clave 'brands' que contenga una lista de strings. Si no encuentras ninguna marca, devuelve una lista vacía. Evita nombres comunes o genéricos a menos que se usen claramente como una marca."},
                {"role": "user", "content": f"Analiza la siguiente transcripción y extrae todas las marcas comerciales mencionadas. Formatea la salida como un objeto JSON con la clave 'brands'. Transcripción:\n\n{transcription_text}"}
            ],
            model="llama-3.1-8b-instant",
            temperature=0.1,
            max_tokens=512,
        ...
        response_content = chat_completion.choices[0].message.content
        data = json.loads(response_content)
        # Asegurarse de devolver una lista, incluso si el JSON es diferente
        if 'brands' in data and isinstance(data['brands'], list):
            return data['brands']
        return []
    except (json.JSONDecodeError, KeyError):
        return ["Error al procesar la respuesta de la IA para marcas."]
    except Exception as e:
        return [f"Error de API al extraer marcas: {str(e)}"]
    ...
    if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True, disabled=not uploaded_file):
        ...
        with st.spinner("🔄 Procesando archivo..."):
            try:
                ...
                st.session_state.transcription = transcription_text; st.session_state.transcription_data = transcription; st.session_state.transcription_id += 1
...
if 'transcription' in st.session_state and 'uploaded_audio_bytes' in st.session_state:
    ...
    if 'brands' in st.session_state: tab_titles.append("🏢 Marcas Clave")
    tabs = st.tabs(tab_titles)
    
    # Índice para acceder a las pestañas dinámicamente
    tab_index = 0
    ...
    if 'people' in st.session_state:
        with tabs[tab_index]:
            ...
            if people_data and not ("Error" in people_data[0]['name']):
                ...
                # --- MEJORA: Bucle corregido para mostrar el contexto de cada persona ---
                for i, person in enumerate(people_data):
                    st.markdown(f"**👤 {person['name']}**")
                    st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;*Rol:* {person.get('role', 'No especificado')}")
                    with st.expander(f"📝 Ver contexto de {person['name']}", expanded=False):
                        st.markdown(f"> {person.get('context', 'Sin contexto disponible.')}")
                    if i < len(people_data) - 1:
    ...
    if 'brands' in st.session_state:
        with tabs[tab_index]:
            tab_index += 1
            st.markdown("### 🏢 Marcas Clave Mencionadas")
            brands_data = st.session_state.get('brands', [])
            if brands_data and not brands_data[0].startswith("Error"):
                st.caption(f"Se identificaron {len(brands_data)} marcas.")
                for brand in brands_data:
                    st.markdown(f"- **{brand}**")
            elif brands_data:
                st.error(f"Ocurrió un error al extraer las marcas: {brands_data[0]}")
            else:
                st.info("🏢 No se identificaron marcas comerciales en el audio.")

    st.markdown("---")
    if st.button("🗑️ Limpiar Todo y Empezar de Nuevo"):
        keys_to_delete = ["transcription", "transcription_data", "uploaded_audio_bytes", "audio_start_time", "summary", "quotes", "last_search", "search_counter", "people", "qa_history", "brands", "transcription_id"]
